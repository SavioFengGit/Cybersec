/* File JavaScript per la ricerca di transazioni su blockchain inserendo l'hash delle misure */
...
// Inseriamo l'address del contratto utilizzando un processo sincrono di immissione di input da tastiera
var measures_hash = readlineSync.question('PLEASE, ENTER MEASURES HASH: ');
...
// Chiamiamo la funzione getIDByMeasuresHash() contenuta nello smart contract passando l'hash delle misure che verrà inserito in input dall'utente; questa funzione restituirà l'ID della transazione relativa all'hash inserito.
contract.methods.getIDByMeasuresHash(measures_hash).call((err, result) => {if (result==0){
																		console.log("\nTHIS HASH HAS NEVER BEEN ENTERED!")
																	}
																	else
																	{console.log("\nTHE ID RELATED TO THIS HASH IS: "+result)}
																	})